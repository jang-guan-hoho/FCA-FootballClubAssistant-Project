...
# 이슈
1. 정기결제 api를 사용하여 카드를 등록해 자동으로 회비를 걷는 시스템을 생각했지만 pg사의 심사를 받지 않으면 정기결제 api를 사용할 수 없다.
2. 결제 방법 차이
  - 클라이언트에서 결제 요청을 처리하는 경우
...
3. 클럽에 가입한 인원만 권한을 줘야한다, 스케쥴이 존재할경우에만 진입을 허용해야했다-네비게이션가드
